...
pub const HARNESS_SURFACES: [(&str, &str, &str, &str, &str); 69] = [
    ...
    (
        ...
        "POST",
        "/api/ollama/pull",
        "operator/admin abortable Ollama pull; SSE progress; never sends num_ctx",
    ),
    (
        "ollama-pull-cancel",
        "/model pull stop",
        ...
        "abort the in-flight Ollama pull",
    ),
    (
        "session",
        "/session",
    ...
    (
        ...
        "bounded FTS5 fact candidates; search is not prompt injection",
    ),
    (
    ...
    (
        ...
        "/memory save <text> :: <reason>",
        "POST",
        "/api/structured-memory/facts",
        "explicitly confirm a private canonical fact; automatic suggestions may remain off",
    ),
    (
    ...
    (
        ...
        "GET",
        "/api/spend/summary",
        "guarded per-provider/model/day inference rollup; dollars at read time only",
    ),
    (
        "spend-predict",
        "(Spend dialog)",
    ...
    (
        ...
        "cancel a persisted schedule; JobStore::finish stays a no-op after job cancel",
    ),
    (
        "agent-status",
        "/agent status",
    ...
    (
        ...
        "POST",
        "/api/web/fetch",
        "read exact allowlisted URLs under one bounded budget; no implicit crawl",
    ),
    ("web-check", "/web check", "POST", "/api/web/check", "check exact URL permissions without fetching or granting access"),
    (
        "web-search",
        "/web search",
        "POST",
    ...
    (
        ...
        "POST",
        "/api/web/research",
        "bounded local-model research with checked citations",
    ),
    (
        "web-research-cancel",
        "/web cancel",
        ...
        "cancel your active research request",
    ),
    (
        "keys",
        "/api",
