...
| POST | `/api/chat/attachments` | Store up to 3 text files (txt/md/json/csv/log, 15 MB each, 64 MB and 512-blob home quota; `attachments.max_concurrent_uploads` bodies in flight, else 503). UUID blobs at `0600`. Inlined only on local chat/`/prompt`, never cloud, `/loop`, or `/agent`. |
