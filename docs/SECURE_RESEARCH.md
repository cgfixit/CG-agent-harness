...
Completion webhooks add optional per-destination bearers, with
`CGAGENTHARNESS_WEBHOOK_TOKEN` kept as the single-owner legacy secret,
in [supported builds](SPEND_AND_NOTIFICATIONS.md). They require restart and do
not select a model or arm coding. The destination is configured separately from
